...
# It returns the original image from the preprocessed image
def deprocess(img):
    img = np.array(img)
...
if uploaded_file is not None:
    ...
    # st.write(len(np.array(image).shape))
    if len(np.array(image).shape) == 2:
        ...
        if st.button('Predict'):
            ...
            # preprocessed_image_array = (preprocessed_image[0].numpy() * 255).astype(np.uint8)
            # st.image(Image.fromarray(preprocessed_image_array.transpose(1, 2, 0)), caption='Preprocessed Image.',
            # width=300)
